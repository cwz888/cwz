...
@section('contents')
    <div class="head-top">
          ...
          <dl>
           ...
           <dd>
            <h1 class="username">{{$u_name}}</h1>
            <ul>
             <li><a href="{{url('/index/prolist')}}"><strong>34</strong><p>全部商品</p></a></li>
    ...
    <form action="#" method="get" class="search">
     ...
     @if($u_name!="请登录")
     <marquee><h3>欢迎{{$u_name}}到来</h3></marquee>
     @else
     ...
     @foreach($brand as $v)
     <ul class="pronav">
        <li><a href="{{url('/index/brand')}}{{$v->brand_id}}">{{$v->brand_name}}</a></li>
     </ul><!--pronav/-->
     @endforeach
     ...
     @foreach($data as $v)
      <div class="index-pro1-list">
       <dl>
        <dt><a href="{{url('/index/proinfo')}}/{{$v->goods_id}}"><img src="{{config('app.img_url')}}{{$v->goods_img}}" /></a></dt>
